...
pg.display.set_caption("Jumping on venus")
background = pg.image.load("afbeeldingen/venusss.jpg").convert()
image = pg.image.load('afbeeldingen/playerShip64.GIF')
rotated_image = pg.transform.rotate(image, 180)
lava = pg.image.load("afbeeldingen/lava.jpg")
...
def win_game():
    ...
    mixer.init()
    mixer.music.load('muziek/ScareCrow_-_Last_Action_Hero_-_Level_2_-_Straight_Mix.mp3')
    ...
    while win_game:
        ...
        Important.button("Try again", 150, 500, 100, 50, green, green_button, screen, black, intro)
...
def game_over():
    ...
    mixer.init()
    mixer.music.load('muziek/DAWilson_-_Turrican_II_Freedom_Blue_Sky_Mix.mp3')
    mixer.music.play(-1, 0)
    ...
    while game_over:
        ...
        text_surf4, text_rect4 = Important.text_objects("So you can try again or try it later", small_text, textcolor)
        text_rect4.center = (width/2, height/2/1.2)
        ...
        Important.button("Try again", 150, 500, 100, 50, green, green_button, screen, black, game_loop)
        Important.button("Quit", 550, 500, 100, 50, red, red_button, screen, black, quit_game)
        pg.display.flip()
        clock.tick(0)
...
def unpaused():
    global paused
    mixer.init()
    mixer.music.load('muziek/Murdock_-_Stardust_Memories_MDK_remix.MP3')
    mixer.music.play(-1, 0)
    paused = False
...
def pause():
    ...
    score = timer.render("Score "+str(point)+" of the 650", 0, (225, 225, 225))
    mixer.init()
    mixer.music.load('muziek/Peter_Gantar_-_Loveland_Mr_Oli.mp3')
    mixer.music.play(-1, 0)
    ...
    while paused:
        ...
        text_surf2, text_rect2 = Important.text_objects("When you click on 'Continue' the game will go on", small_text, textcolor)
        text_rect2.center = (width/2, height/2/2)

        text_surf4, text_rect4 = Important.text_objects("If you click on 'restart' the game start from the beginning", small_text, textcolor)
        text_rect4.center = (width/2, height/2/1.7)

        text_surf3, text_rect3 = Important.text_objects("If you click on 'Quit' the game will end", small_text, textcolor)
        text_rect3.center = (width/2, height/2/1.5)
...
def addplatform():
    np = {'rect': pg.Rect(random.randint(10, width - 250), 0, 200, 5)}
...
def intro():
    mixer.init()
    mixer.music.load('muziek/DeAdLy_cOoKiE_-_Odyssey.mp3')
    mixer.music.play(-1, 0)
    ...
    while True:
        ...
        text_surf9, text_rect9 = Important.text_objects("The head (platforms) move down. You can fly out of the window Left = right", small_text, textcolor)
